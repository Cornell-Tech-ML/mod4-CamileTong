...
def test_conv1d_simple() -> None:
    ...
    out = minitorch.Conv1dFun.apply(t, t2)  # type: ignore
...
def test_conv1d(input: Tensor, weight: Tensor) -> None:
    print(input, weight)
    minitorch.grad_check(minitorch.Conv1dFun.apply, input, weight)  # type: ignore
...
def test_conv(input: Tensor, weight: Tensor) -> None:
    minitorch.grad_check(minitorch.Conv2dFun.apply, input, weight)  # type: ignore
...
def test_conv2() -> None:
    ...
    out = minitorch.Conv2dFun.apply(t, t2)  # type: ignore
    out.sum().backward()

    minitorch.grad_check(minitorch.Conv2dFun.apply, t, t2)  # type: ignore
